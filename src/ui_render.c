...
bool text_init(SDL_Renderer *r, const char *font_path) {
    ...
    snprintf(png_path, sizeof(png_path), "./Data/font_atlas.png");
    snprintf(bin_path, sizeof(bin_path), "./Data/font_atlas.bin");
    FILE *mf = fopen(bin_path, "rb");
    if (!mf) {
...
#ifdef MUSE_GPU
    if (gpu_active()) {
        gpu_emit_rrect_grad(x, y, w, h, 0.0f, 1,
...
#ifdef MUSE_GPU
    if (gpu_active()) {
        gpu_emit_rrect(x, y, w, h, radius, cr, cg, cb, ca);
        return;
    }
#endif
    if (radius < 1.0f) {
...
#ifdef MUSE_GPU
    if (gpu_active()) {
        gpu_emit_note(x, y, w, h, radius, cr, cg, cb, ca);
        return;
    }
#endif
    if (radius < 1.0f) {
...
#ifdef MUSE_GPU
    if (gpu_active()) {
        gpu_emit_rrect_grad(x, y, w, h, radius, 0,
                            tr, tg, tb, ta, br_, bg_, bb_, ba_);
        return;
    }
#endif
    if (radius < 1.0f) {
...
#ifdef MUSE_GPU
    if (gpu_active()) {
        gpu_emit_rrect_outline(x, y, w, h, radius, 1.0f,
                               cr, cg, cb, 255);
        return;
    }
#endif
...
#ifdef MUSE_GPU
    if (gpu_active()) {
        gpu_emit_rrect(cx - radius, cy - radius, radius * 2.0f,
                       radius * 2.0f, radius, cr, cg, cb, 255);
        return;
    }
#endif
...
#ifdef MUSE_GPU
    if (gpu_active()) {
        gpu_emit_rrect_outline(cx - radius, cy - radius, radius * 2.0f,
                               radius * 2.0f, radius, 1.0f,
                               cr, cg, cb, 255);
        return;
    }
#endif
...
void draw_dropdown_arrow(SDL_Renderer *r, float cx, float cy, float size,
                         uint8_t cr, uint8_t cg, uint8_t cb) {
    draw_dropdown_arrow_dir(r,cx,cy,size,false,cr,cg,cb);
}

void draw_dropdown_arrow_dir(SDL_Renderer *r, float cx, float cy, float size,
                             bool upward, uint8_t cr, uint8_t cg, uint8_t cb) {
    ...
    for (int i = 0; i <= steps; i++) {
        ...
        if (w < 0.3f) w = 0.3f;
        SDL_FRect strip = { cx - w, cy + (upward ? -dy : dy), w * 2, 0.25f };
        SDL_RenderFillRect(r, &strip);
    }
}
