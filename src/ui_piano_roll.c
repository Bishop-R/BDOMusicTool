...
static int velpoint_x_cmp(const void*a,const void*b){float d=((const VelPoint*)a)->x-((const VelPoint*)b)->x;return(d>0)-(d<0);}
static int velpoint_height_cmp(const void*a,const void*b){float d=((const VelPoint*)b)->y_top-((const VelPoint*)a)->y_top;return(d>0)-(d<0);}
...
void ui_piano_roll_render(MuseApp *app) {
    ...
    int first_beat = (int)floor(muse_project_ms_to_beat(&app->project,start_ms));
    ...
    if (inst) {
        ...
        if (exceed_ms >= 0) {
            float ex_x = app_ms_to_x(app,exceed_ms);
            if (ex_x < rx + rw) {
    ...
    /* Subdivision lines first (lightest) */
    {
        ...
        if (subdiv > 1) {
            int first_sub = (int)floor(muse_project_ms_to_beat(&app->project,start_ms)*subdiv);
            if (first_sub < 0) first_sub = 0;
            for (int s = first_sub; ; s++) {
                double ms = muse_project_beat_to_ms(&app->project,(double)s/subdiv);
                float x = app_ms_to_x(app,ms);
                if (x > rx + rw) break;
                if (x < rx) continue;
                if (s % subdiv != 0)
    ...
    for (int b = first_beat; ; b++) {
        double ms = muse_project_beat_to_ms(&app->project,(double)b);
        float x = app_ms_to_x(app,ms);
        if (x > rx + rw) break;
        if (x < rx) continue;
    ...
    double vis_end_ms = app_x_to_ms(app,rx+rw);
    ...
    for (int li = 0; li < app->project.num_layers; li++) {
        ...
        for (int si = 0; si < ly->num_sublayers; si++) {
            ...
            for (int n = 0; n < na->count; n++) {
                ...
                float nx = app_ms_to_x(app,note->start);
                float nw = app_ms_to_x(app,note->start+note->dur)-nx;
                if (nw < 4) nw = 4;
    ...
    if (app->looping) {
        float lx0 = app_ms_to_x(app,app->loop_start_ms);
        float lx1 = app_ms_to_x(app,app->loop_end_ms);
    ...
    /* playhead line */
    {
        float px_2d = app_ms_to_x(app,app->playhead_ms);
    ...
    if (persp_t > 0.99f) goto skip_timeline_header;
    {
        ...
        /* Editor-only arrangement markers; BDO export intentionally ignores them. */
        for(int i=0;i<app->project.num_markers;i++){
            float mx=app_ms_to_x(app,app->project.markers[i].time_ms);
            if(mx>=rx&&mx<=rx+rw){draw_vline(r,mx,hdr_y,hdr_y+HEADER_HEIGHT,COL_GOLD);draw_text_bold(r,app->project.markers[i].label,mx+3,hdr_y+2,8,COL_GOLD);}
        }

        /* Playhead in header */
        {
            float px = app_ms_to_x(app,app->playhead_ms);
            if (px >= rx && px <= rx + rw) {
        ...
        {
            float bpm_cx=keys_x+KEYS_WIDTH*0.5f;
            draw_text_centered(r,"BPM",bpm_cx-4,hdr_y+HEADER_HEIGHT*0.5f,
                               8,app->tempo_lane_open?COL_GOLD:COL_TEXT_DIM);
            draw_dropdown_arrow_dir(r,bpm_cx+14,hdr_y+HEADER_HEIGHT*0.5f,7,
                                    app->tempo_lane_open,
                                    app->tempo_lane_open?COL_GOLD:COL_TEXT_DIM);
        }
        if(app->tempo_lane_anim>0.01f){
            float top=hdr_y+HEADER_HEIGHT,bottom=hdr_y+app_timeline_header_h(app),lh=bottom-top;
            draw_filled_rect(r,keys_x,top,KEYS_WIDTH,lh,COL_BG_DARK,255);
            draw_filled_rect(r,rx,top,rw,lh,COL_BG_DARK,255);
            push_clip(r,rx,top,rw,lh);
            static const int guides[]={40,60,90,120,180,240,360};
            for(int gi=0;gi<7;gi++){float gy=app_tempo_editor_bpm_to_y(app,(float)guides[gi]);draw_hline(r,rx,rx+rw,gy,0x2B,0x30,0x39);char s[8];snprintf(s,sizeof(s),"%d",guides[gi]);draw_text(r,s,rx+4,gy-8,7,0x7F,0x88,0x98);}
            double startbeat=muse_project_ms_to_beat(&app->project,start_ms);
            float prevx=rx+(float)((0.0-startbeat)*bpx);
            float prevb=(float)app->project.bpm;
            float prevy=app_tempo_editor_bpm_to_y(app,prevb);
            bool prevramp=false;
            float prevcurve=0;
            for(int i=0;i<app->project.num_tempo_points;i++){
                MuseTempoPoint*t=&app->project.tempo_points[i];float nx=rx+(float)((t->beat-startbeat)*bpx);float ny=app_tempo_editor_bpm_to_y(app,t->bpm);
                SDL_FColor ft={1,.42f,.16f,.20f},fb={1,.42f,.16f,.025f};int vi[6]={0,1,2,0,2,3};
                if(prevramp){float sx=prevx,sy=prevy;for(int sm=1;sm<=20;sm++){double u=sm/20.0,sh=u;if(prevcurve>0)sh=1-pow(1-u,1+4*prevcurve);else if(prevcurve<0)sh=pow(u,1-4*prevcurve);float ex=prevx+(nx-prevx)*(float)u,ey=app_tempo_editor_bpm_to_y(app,prevb+(t->bpm-prevb)*(float)sh);SDL_Vertex v[4]={0};v[0].position=(SDL_FPoint){sx,sy};v[0].color=ft;v[1].position=(SDL_FPoint){ex,ey};v[1].color=ft;v[2].position=(SDL_FPoint){ex,bottom};v[2].color=fb;v[3].position=(SDL_FPoint){sx,bottom};v[3].color=fb;SDL_RenderGeometry(r,NULL,v,4,vi,6);sx=ex;sy=ey;}}
                else{SDL_Vertex v[4]={0};v[0].position=(SDL_FPoint){prevx,prevy};v[0].color=ft;v[1].position=(SDL_FPoint){nx,prevy};v[1].color=ft;v[2].position=(SDL_FPoint){nx,bottom};v[2].color=fb;v[3].position=(SDL_FPoint){prevx,bottom};v[3].color=fb;SDL_RenderGeometry(r,NULL,v,4,vi,6);}
                SDL_SetRenderDrawColor(r,0xFF,0x98,0x50,0xB8);if(prevramp){float sx=prevx,sy=prevy;for(int sm=1;sm<=20;sm++){double u=sm/20.0,sh=u;if(prevcurve>0)sh=1-pow(1-u,1+4*prevcurve);else if(prevcurve<0)sh=pow(u,1-4*prevcurve);float ex=prevx+(nx-prevx)*(float)u,ey=app_tempo_editor_bpm_to_y(app,prevb+(t->bpm-prevb)*(float)sh);SDL_RenderLine(r,sx,sy,ex,ey);sx=ex;sy=ey;}}else{SDL_RenderLine(r,prevx,prevy,nx,prevy);SDL_RenderLine(r,nx,prevy,nx,ny);}draw_circle_filled(r,nx,ny,(app->tempo_selected_idx==i||app->tempo_node_drag_idx==i)?6:4,COL_GOLD_BRIGHT);if(app->tempo_lane_anim>.6f){char s[24];snprintf(s,sizeof(s),"%.1f BPM%s",t->bpm,t->ramp_to_next?" ~>":"");draw_text(r,s,nx+7,ny-10,8,COL_GOLD_LIGHT);}prevx=nx;prevy=ny;prevb=t->bpm;prevramp=t->ramp_to_next!=0;prevcurve=t->curve_to_next;
            }
            SDL_FColor ft={1,.42f,.16f,.20f},fb={1,.42f,.16f,.025f};SDL_Vertex v[4]={0};v[0].position=(SDL_FPoint){prevx,prevy};v[0].color=ft;v[1].position=(SDL_FPoint){rx+rw,prevy};v[1].color=ft;v[2].position=(SDL_FPoint){rx+rw,bottom};v[2].color=fb;v[3].position=(SDL_FPoint){prevx,bottom};v[3].color=fb;int vi[6]={0,1,2,0,2,3};SDL_RenderGeometry(r,NULL,v,4,vi,6);SDL_SetRenderDrawColor(r,0xFF,0x98,0x50,0xB8);SDL_RenderLine(r,prevx,prevy,rx+rw,prevy);
            if(app->tempo_lane_anim>.8f)for(int i=0;i+1<app->project.num_tempo_points;i++){MuseTempoPoint*a=&app->project.tempo_points[i],*b=&app->project.tempo_points[i+1];if(!a->ramp_to_next)continue;double sh=.5;if(a->curve_to_next>0)sh=1-pow(.5,1+4*a->curve_to_next);else if(a->curve_to_next<0)sh=pow(.5,1-4*a->curve_to_next);float hx=rx+(float)(((a->beat+b->beat)*.5-startbeat)*bpx),hy=app_tempo_editor_bpm_to_y(app,a->bpm+(b->bpm-a->bpm)*(float)sh);draw_circle_filled(r,hx,hy,app->tempo_curve_dragging&&app->tempo_curve_drag_idx==i?5:3.5f,0xE0,0xE4,0xEA);}
            pop_clip(r);draw_hline(r,keys_x,(float)app->win_w,bottom,COL_BORDER);
        }
    ...
    /* velocity pane - only shows when notes are selected, FL Studio style */
    {
        ...
        if (sel_count > 0) {
            ...
            VelPoint *pts = malloc((size_t)sel_count * sizeof(*pts));
            int npts = 0;
            ...
            /* Sort by x position */
            qsort(pts, (size_t)npts, sizeof(*pts), velpoint_x_cmp);
            ...
            for (int i = 0; i < npts - 1; i++) {
                ...
                /* Match the tempo lane's orange graph language. */
                const uint8_t fc_r = 0xFF, fc_g = 0x6B, fc_b = 0x29;
                /* Trapezoid as 2 triangles: (x1,y1) (x2,y2) (x2,bottom) (x1,bottom) */
                SDL_FColor fc_top = { fc_r / 255.0f, fc_g / 255.0f, fc_b / 255.0f, 0.20f };
                SDL_FColor fc_bottom = { fc_r / 255.0f, fc_g / 255.0f, fc_b / 255.0f, 0.025f };
                SDL_Vertex verts[4] = {
                    { .position={x1, y1},       .color=fc_top },
                    { .position={x2, y2},       .color=fc_top },
                    { .position={x2, bottom_y}, .color=fc_bottom },
                    { .position={x1, bottom_y}, .color=fc_bottom },
                };
            ...
            for (int p = 0; p < npts; p++) {
                ...
                /* Keep the per-note alternating palette: it prevents dense
                   stem groups from collapsing into one orange picket fence. */
                const MuseTechnique *tech = technique_by_id(note->ntype);
                int tcr = tech ? tech->r : 120;
                int tcg = tech ? tech->g : 140;
                int tcb = tech ? tech->b : 170;
                ...
                /* Selection remains a neutral bright override. */
                ...
                if (is_vel_selected || is_drag_target) {
                    line_r = 0xFF; line_g = 0xF0; line_b = 0xD6;
                } else {
                ...
                else if (is_vel_selected)
                    draw_circle_filled(r, stem_cx, y_top, 5, COL_GOLD_LIGHT);
                else
            ...
            if (app->vel_dragging && app->vel_drag_note_idx >= 0 &&
                ...
                float dnx = app_ms_to_x(app,dn->start);
    ...
    /* horizontal scrollbar */
    {
        float sb_w = 14;
        float sb_h = 14;
        float total_ms_w = (float)muse_project_beat_to_ms(&app->project,(double)app->measures*beats_per_measure);
        float total_px = (float)(app->measures*beats_per_measure*bpx);
        ...
        float scroll_frac_h = (float)(muse_project_ms_to_beat(&app->project,app->scroll_x)/(app->measures*beats_per_measure));
