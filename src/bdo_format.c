...
typedef struct { MuseNote note; int order; } BdoExportNote;
static int export_note_cmp(const void *a,const void *b){
    const BdoExportNote*x=a,*y=b;
    if(x->note.start<y->note.start)return -1;if(x->note.start>y->note.start)return 1;
...
int bdo_save(const char *path, const MuseProject *proj) {
    ...
    for (int i = 0; i < proj->num_layers; i++) {
        ...
        if(total>MAX_NOTES_PER_INSTRUMENT)total=MAX_NOTES_PER_INSTRUMENT;
    ...
    /* BDO's in-game tempo field is limited to 200 BPM. Virtual tempo-map
       values above that are already baked into note/pause timing. */
    uint16_t export_bpm = proj->bpm;
    if (export_bpm < 1) export_bpm = 120;
    if (export_bpm > 200) export_bpm = 200;
    write_u16(pt + 0x84, export_bpm);
    ...
    for (int g = 0; g < proj->num_layers; g++) {
        ...
        int authored=muse_layer_note_count(ly),total_notes=authored;
        BdoExportNote *export_notes=authored?malloc((size_t)authored*sizeof(*export_notes)):NULL;
        if(authored&&!export_notes){free(pt);return -1;}
        int ei=0;
        for(int s=0;s<ly->num_sublayers;s++)for(int n=0;n<ly->sublayers[s].count;n++){
            export_notes[ei]=(BdoExportNote){ly->sublayers[s].notes[n],ei};ei++;
        }
        qsort(export_notes,(size_t)authored,sizeof(*export_notes),export_note_cmp);
        if(total_notes>MAX_NOTES_PER_INSTRUMENT)total_notes=MAX_NOTES_PER_INSTRUMENT;
        ...
        for (int t = 0; t < num_data_tracks; t++) {
            ...
            for(int written=0;written<chunk;written++){
                    const MuseNote *note=&export_notes[notes_written].note;
