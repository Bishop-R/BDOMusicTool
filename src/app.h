...
typedef struct MuseApp {
    ...
    bool     tempo_lane_open;
    float    tempo_lane_anim;
    bool     tempo_node_dragging;
    int      tempo_node_drag_idx;
    MuseProject tempo_node_drag_before;
    bool     tempo_node_drag_changed;
    int      tempo_selected_idx;
    bool     tempo_curve_dragging;
    int      tempo_curve_drag_idx;
    MuseProject tempo_curve_drag_before; /* shallow snapshot; tempo map is embedded */
    bool     tempo_curve_drag_changed;
    ...
    /* Adjustable humanization dialog (middle-click note menu). */
    bool   humanize_open;
    int    humanize_timing;   /* +/- milliseconds, 0..50 */
    int    humanize_velocity; /* +/- MIDI velocity, 0..32 */
    int    humanize_length;   /* +/- percent, 0..40 */
    int    humanize_drag_slider; /* 0=none, 1=timing, 2=velocity, 3=length */
    int    humanize_edit_field;  /* 0=none, 1..3 */
    char   humanize_edit_buf[8];
    UiRect _humanize_sliders[3];
    UiRect _humanize_fields[3];

    /* Exact velocity dialog opened from the note context menu. */
    bool   velocity_set_open;
    int    velocity_set_value;
    bool   velocity_set_dragging;
    bool   velocity_set_editing;
    char   velocity_set_buf[8];
    UiRect _velocity_set_slider;
    UiRect _velocity_set_field;
    ...
    /* file */
    char   filename[256];
    char   project_path[512]; /* current editable .composer file; empty until saved */
    ...
    bool midi_recording;
    double midi_note_start[128];
    uint8_t midi_note_velocity[128];
...
float  app_timeline_header_h(const MuseApp *app);
float  app_tempo_editor_bpm_to_y(const MuseApp *app,float bpm);
float  app_tempo_editor_y_to_bpm(const MuseApp *app,float y);
