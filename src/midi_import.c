...
static int tempo_event_cmp(const void*a,const void*b){const TempoEvent*x=a,*y=b;return(x->tick>y->tick)-(x->tick<y->tick);}
...
MidiImportData *midi_parse(const char *path) {
    ...
    qsort(g_tempo_map,(size_t)g_tempo_map_count,sizeof(g_tempo_map[0]),tempo_event_cmp);
...
void midi_apply(MidiImportData *mid, MuseProject *out) {
    ...
    out->bpm = mid->bpm > 200 ? 200 : mid->bpm;
    if(out->bpm<20)out->bpm=120;
    if (mid->time_sig >= 2 && mid->time_sig <= 8)
        out->time_sig = mid->time_sig;

    /* Preserve MIDI tempo events as editor-only graph points. Notes are
       converted with the same step map below, so BDO export receives the
       identical timing while keeping its single <=200 BPM header value. */
    if(mid->tpqn>0){
        for(int i=0;i<g_tempo_map_count;i++){
            double beat=(double)g_tempo_map[i].tick/mid->tpqn;
            double uspq=g_tempo_map[i].us_per_tick*mid->tpqn;
            float bpm=uspq>0?(float)(60000000.0/uspq):(float)mid->bpm;
            if(bpm<20)bpm=20;if(bpm>400)bpm=400;
            muse_project_set_tempo_at_beat(out,beat,bpm);
        }
    }
