...
#define AUTOSAVE_GENERATIONS 3
static void autosave_path(char *out,size_t cap,int generation){
    if(generation==0)snprintf(out,cap,"Save/Autosave/autosave.composer");
    else snprintf(out,cap,"Save/Autosave/autosave.composer.%d",generation);
}
static void autosave_remove_all(void){char p[128];for(int i=0;i<=AUTOSAVE_GENERATIONS;i++){autosave_path(p,sizeof(p),i);remove(p);}}
static int autosave_write(MuseApp*app){
    char from[128],to[128];autosave_path(to,sizeof(to),AUTOSAVE_GENERATIONS);remove(to);
    for(int i=AUTOSAVE_GENERATIONS-1;i>=0;i--){autosave_path(from,sizeof(from),i);autosave_path(to,sizeof(to),i+1);rename(from,to);}
    autosave_path(to,sizeof(to),0);return muse_save(to,app);
}
...
bool muse_app_init(MuseApp *app) {
    ...
    app->hover_layer     = -1;
    app->hover_btn       = -1;
    app->tempo_selected_idx = -1;
    app->tempo_node_drag_idx = -1;
    app->tempo_curve_drag_idx = -1;
    ...
    app->renderer = NULL;
#ifdef MUSE_GPU
    if (gpu_init(app->window)) {
        SDL_Log("Rendering with SDL_GPU");
    } else {
        SDL_Log("SDL_GPU unavailable (%s); using SDL_Renderer", SDL_GetError());
        gpu_shutdown();
    }
#endif
    if (!gpu_active()) {
    ...
    /* Text uses the packaged pre-baked atlas; no system font is required. */
    bool font_ok = text_init(app->renderer, NULL);
    if (!font_ok) SDL_Log("Warning: no font loaded - text will not render");

    /* load instrument icons */
    {
        const char *icon_tries[] = { "./Data/Icons", NULL };
        ...
        if (!icons_loaded)
            SDL_Log("Instrument icons not found in packaged Data/Icons");
    }

    /* find samples dir and extract from BDO if needed */
    {
        const char *samples_dir = NULL;
        const char *tries[] = { "./Data/samples", NULL };
        ...
        /* A clean packaged build always owns its sample directory. Do not
           borrow Muse's extracted assets: that suppresses this project's
           first-run setup and couples the two applications together. */
        if (!samples_dir) {
            SDL_CreateDirectory("Data");
            SDL_CreateDirectory("Data/samples");
            samples_dir = "./Data/samples";
        }

        /* Check if actual sample .ogg files exist (not just key_zones.json) */
        bool need_extraction = true;
        if (samples_dir) {
            char probe[512];
                ...
                while (waiting) {
                    ...
                    while (SDL_PollEvent(&ev)) {
                        if (ev.type == SDL_EVENT_QUIT) {
                            ...
                            /* Consume the setup-screen Escape press here so it
                               cannot also dismiss the account-link overlay. */
                            waiting = false;
    ...
    if(!midi_input_init()) SDL_Log("No hardware MIDI input available");
    ...
    {
        char as_path[128]; autosave_path(as_path,sizeof(as_path),0);
        FILE *test = NULL;
        for(int generation=0;generation<=AUTOSAVE_GENERATIONS&&!test;generation++){autosave_path(as_path,sizeof(as_path),generation);test=fopen(as_path,"rb");}
        if (test) {
            fclose(test);
            ...
            if (SDL_ShowMessageBox(&mbd, &btn) && btn == 1) {
                int recovery_rc=-1;
                for(int generation=0;generation<=AUTOSAVE_GENERATIONS;generation++){autosave_path(as_path,sizeof(as_path),generation);if((recovery_rc=muse_load(as_path,app))==0)break;}
                if (recovery_rc == 0) {
...
float app_timeline_header_h(const MuseApp *app){return HEADER_HEIGHT+112.0f*app->tempo_lane_anim;}
float app_roll_y(const MuseApp *app) {
    return (float)(TRANSPORT_H+2)+app_timeline_header_h(app);
}
...
float app_roll_h(const MuseApp *app) {
    ...
    return (float)(app->win_h-TRANSPORT_H-2-STATUS_BAR_H)-app_timeline_header_h(app)-vel_h;
}
float app_tempo_editor_bpm_to_y(const MuseApp*app,float bpm){const float lo=20,hi=400;if(bpm<lo)bpm=lo;if(bpm>hi)bpm=hi;float top=TRANSPORT_H+2+HEADER_HEIGHT+3,bottom=TRANSPORT_H+2+app_timeline_header_h(app)-3;if(bottom<=top)return bottom;float t=logf(bpm/lo)/logf(hi/lo);return bottom-t*(bottom-top);}
float app_tempo_editor_y_to_bpm(const MuseApp*app,float y){const float lo=20,hi=400;float top=TRANSPORT_H+2+HEADER_HEIGHT+3,bottom=TRANSPORT_H+2+app_timeline_header_h(app)-3;if(bottom<=top)return lo;float t=(bottom-y)/(bottom-top);if(t<0)t=0;if(t>1)t=1;return lo*powf(hi/lo,t);}
...
double app_x_to_ms(const MuseApp *app, float x) {
    ...
    double beat=muse_project_ms_to_beat(&app->project,app->scroll_x)+(x-rx)/beat_px(app);
    return muse_project_beat_to_ms(&app->project,beat);
}

float app_ms_to_x(const MuseApp *app, double ms) {
    double beat=muse_project_ms_to_beat(&app->project,ms);
    double start=muse_project_ms_to_beat(&app->project,app->scroll_x);
    return app_roll_x(app)+(float)((beat-start)*beat_px(app));
}
...
double app_snap_ms(const MuseApp *app, double ms) {
    ...
    double beat=muse_project_ms_to_beat(&app->project,ms);
    return muse_project_beat_to_ms(&app->project,round(beat*grid_div(app))/grid_div(app));
}

static double app_snap_ms_floor(const MuseApp *app, double ms) {
    if (app->grid_snap == GRID_FREE) return ms;
    double beat=muse_project_ms_to_beat(&app->project,ms);
    return muse_project_beat_to_ms(&app->project,floor(beat*grid_div(app))/grid_div(app));
}

/* Fine live-drag snap for gridded modes. Free must preserve exact timing. */
static double app_snap_ms_64(const MuseApp *app, double ms) {
    if (app->grid_snap == GRID_FREE) return ms;
    double beat=muse_project_ms_to_beat(&app->project,ms);
    return muse_project_beat_to_ms(&app->project,round(beat*16.0)/16.0);
}
...
static void auto_expand_measures(MuseApp *app) {
    ...
    int needed = (int)(muse_project_ms_to_beat(&app->project,max_end) / ts) + 2;
...
static void app_change_base_bpm(MuseApp *app,uint16_t bpm){
    double ph=muse_project_ms_to_beat(&app->project,app->playhead_ms),sc=muse_project_ms_to_beat(&app->project,app->scroll_x);
    double la=muse_project_ms_to_beat(&app->project,app->loop_start_ms),lb=muse_project_ms_to_beat(&app->project,app->loop_end_ms);
    undo_push(&app->project);if(muse_project_set_base_bpm(&app->project,bpm)){app->playhead_ms=muse_project_beat_to_ms(&app->project,ph);app->scroll_x=muse_project_beat_to_ms(&app->project,sc);app->loop_start_ms=muse_project_beat_to_ms(&app->project,la);app->loop_end_ms=muse_project_beat_to_ms(&app->project,lb);restart_playback_if_playing(app);}
}
...
/* Middle-click note menu: only operations that BDO can represent faithfully. */
enum {
    NM_TRANSPOSE_UP = 1, NM_TRANSPOSE_DOWN, NM_OCTAVE_UP, NM_OCTAVE_DOWN,
    NM_LENGTH_HALF = 10, NM_LENGTH_DOUBLE,
    NM_VELOCITY_UP = 20, NM_VELOCITY_DOWN, NM_VELOCITY_UP_10,
    NM_VELOCITY_DOWN_10, NM_VELOCITY_RANDOM, NM_VELOCITY_SET,
    NM_QUANTIZE = 30, NM_HUMANIZE, NM_STRUM_UP, NM_STRUM_DOWN,
    NM_CHORDIFY = 40, NM_DELETE = 50
};
static const UiMenuItem NOTE_TRANSPOSE_MENU[] = {
    { "+1 Semitone", NM_TRANSPOSE_UP, "Shift+Up", false, NULL, 0 },
    { "-1 Semitone", NM_TRANSPOSE_DOWN, "Shift+Down", false, NULL, 0 },
    { "+1 Octave", NM_OCTAVE_UP, "Ctrl+Shift+Up", false, NULL, 0 },
    { "-1 Octave", NM_OCTAVE_DOWN, "Ctrl+Shift+Down", false, NULL, 0 },
};
static const UiMenuItem NOTE_LENGTH_MENU[] = {
    { "Half", NM_LENGTH_HALF, NULL, false, NULL, 0 },
    { "Double", NM_LENGTH_DOUBLE, NULL, false, NULL, 0 },
};
static const UiMenuItem NOTE_VELOCITY_MENU[] = {
    { "+1", NM_VELOCITY_UP, "Up", false, NULL, 0 },
    { "-1", NM_VELOCITY_DOWN, "Down", false, NULL, 0 },
    { "+10", NM_VELOCITY_UP_10, "Ctrl+Up", false, NULL, 0 },
    { "-10", NM_VELOCITY_DOWN_10, "Ctrl+Down", false, NULL, 0 },
    { NULL, 0, NULL, false, NULL, 0 },
    { "Set exact value...", NM_VELOCITY_SET, NULL, false, NULL, 0 },
    { "Randomize (+/-20)", NM_VELOCITY_RANDOM, NULL, false, NULL, 0 },
};
static const UiMenuItem NOTE_STRUM_MENU[] = {
    { "Up (low to high)", NM_STRUM_UP, NULL, false, NULL, 0 },
    { "Down (high to low)", NM_STRUM_DOWN, NULL, false, NULL, 0 },
};
static const UiMenuItem NOTE_MENU[] = {
    { "Transpose", 0, NULL, false, NOTE_TRANSPOSE_MENU, 4 },
    { "Length", 0, NULL, false, NOTE_LENGTH_MENU, 2 },
    { "Velocity", 0, NULL, false, NOTE_VELOCITY_MENU, 7 },
    { NULL, 0, NULL, false, NULL, 0 },
    { "Quantize", NM_QUANTIZE, "Ctrl+Alt+Q", false, NULL, 0 },
    { "Humanize...", NM_HUMANIZE, "Ctrl+Alt+H", false, NULL, 0 },
    { "Strum", 0, NULL, false, NOTE_STRUM_MENU, 2 },
    { NULL, 0, NULL, false, NULL, 0 },
    { "Chordify...", NM_CHORDIFY, NULL, false, NULL, 0 },
    { NULL, 0, NULL, false, NULL, 0 },
    { "Delete", NM_DELETE, "Del", false, NULL, 0 },
};

static int selected_note_count(NoteArray *na) {
    int n = 0;
    if (na) for (int i = 0; i < na->count; i++) n += na->notes[i].selected != 0;
    return n;
}
static void menu_transpose(MuseApp *app, int semitones) {
    NoteArray *na = app_active_notes(app); int count = selected_note_count(na);
    if (!count) return;
    undo_push(&app->project);
    for (int i = 0; i < na->count; i++) if (na->notes[i].selected) {
        int pitch = (int)na->notes[i].pitch + semitones;
        if (pitch < PITCH_MIN) pitch = PITCH_MIN;
        if (pitch > PITCH_MAX) pitch = PITCH_MAX;
        na->notes[i].pitch = (uint8_t)pitch;
    }
    app->project.dirty = true;
    set_status_msg(app, "Transposed %d note%s by %+d", count, count == 1 ? "" : "s", semitones);
}
static void menu_scale_length(MuseApp *app, double factor) {
    NoteArray *na = app_active_notes(app); int count = selected_note_count(na);
    if (!count) return;
    undo_push(&app->project);
    for (int i = 0; i < na->count; i++) if (na->notes[i].selected) {
        na->notes[i].dur *= factor;
        if (na->notes[i].dur < 10.0) na->notes[i].dur = 10.0;
    }
    app->project.dirty = true; auto_expand_measures(app);
    set_status_msg(app, "Scaled length of %d note%s by %.2gx", count, count == 1 ? "" : "s", factor);
}
static void menu_velocity(MuseApp *app, int delta, bool randomize) {
    NoteArray *na = app_active_notes(app); int count = selected_note_count(na);
    if (!count) return;
    undo_push(&app->project);
    for (int i = 0; i < na->count; i++) if (na->notes[i].selected) {
        int d = randomize ? (rand() % 41) - 20 : delta;
        int velocity = (int)na->notes[i].vel + d;
        if (velocity < 1) velocity = 1;
        if (velocity > 127) velocity = 127;
        na->notes[i].vel = (uint8_t)velocity;
    }
    app->project.dirty = true;
    if (randomize) set_status_msg(app, "Randomized velocity on %d notes", count);
    else set_status_msg(app, "Velocity %+d on %d notes", delta, count);
}
static void note_menu_cb(int id, void *user) {
    MuseApp *app = user; NoteArray *na = app_active_notes(app); int changed = 0;
    switch (id) {
    case NM_TRANSPOSE_UP: menu_transpose(app, 1); break;
    case NM_TRANSPOSE_DOWN: menu_transpose(app, -1); break;
    case NM_OCTAVE_UP: menu_transpose(app, 12); break;
    case NM_OCTAVE_DOWN: menu_transpose(app, -12); break;
    case NM_LENGTH_HALF: menu_scale_length(app, .5); break;
    case NM_LENGTH_DOUBLE: menu_scale_length(app, 2.0); break;
    case NM_VELOCITY_UP: menu_velocity(app, 1, false); break;
    case NM_VELOCITY_DOWN: menu_velocity(app, -1, false); break;
    case NM_VELOCITY_UP_10: menu_velocity(app, 10, false); break;
    case NM_VELOCITY_DOWN_10: menu_velocity(app, -10, false); break;
    case NM_VELOCITY_RANDOM: menu_velocity(app, 0, true); break;
    case NM_VELOCITY_SET: {
        int total=0,count=0;
        if(na)for(int i=0;i<na->count;i++)if(na->notes[i].selected){total+=na->notes[i].vel;count++;}
        app->velocity_set_value=count?(total+count/2)/count:100;
        app->velocity_set_open=true;app->velocity_set_dragging=false;app->velocity_set_editing=false;
        break;
    }
    case NM_QUANTIZE:
        undo_push(&app->project); changed = notes_quantize(na, ms_per_beat(app), grid_div(app), 100, 50, false);
        set_status_msg(app, "Quantized: %d notes", changed); goto tool_done;
    case NM_HUMANIZE:
        app->humanize_open = true;
        if (app->humanize_timing == 0 && app->humanize_velocity == 0 && app->humanize_length == 0) {
            app->humanize_timing = 12; app->humanize_velocity = 8; app->humanize_length = 10;
        }
        break;
    case NM_STRUM_UP: case NM_STRUM_DOWN:
        undo_push(&app->project);
        changed = notes_strum(na, 25, 4, 2, (uint32_t)SDL_GetTicks(), id == NM_STRUM_UP);
        set_status_msg(app, "Strummed %s: %d notes", id == NM_STRUM_UP ? "up" : "down", changed);
        goto tool_done;
    case NM_CHORDIFY: app->chord_open = true; app->chord_scroll = 0; app->chord_hover = -1; break;
    case NM_DELETE: delete_selected(app); break;
    }
    return;
tool_done:
    if (changed) { app->project.dirty = true; auto_expand_measures(app); restart_playback_if_playing(app); }
}
...
static void handle_left_panel_click(MuseApp *app, float mx, float my) {
    ...
    if (ui_rect_contains(app->_lp_export_bdo, mx, my)) {
        ...
        app->project.effector_chorus_depth = (uint8_t)app->fx_chorus_depth;
        app->project.effector_chorus_freq  = (uint8_t)app->fx_chorus_freq;
        BdoValidation validation = bdo_validate_project(&app->project);
        if (validation.errors > 0 || validation.warnings > 0) {
            char report[512];
            bdo_validation_message(&app->project, report, sizeof(report));
            SDL_ShowSimpleMessageBox(validation.errors ? SDL_MESSAGEBOX_ERROR : SDL_MESSAGEBOX_WARNING,
                                     "BDO Export Validation", report, app->window);
            if (validation.errors > 0) {
                set_status_msg(app, "BDO export blocked: fix %d validation error(s)", validation.errors);
                return;
            }
        }
...
static void handle_mouse_down_roll(MuseApp *app, float mx, float my, int button) {
    if (button == SDL_BUTTON_MIDDLE) {
        /* Note/selection opens the context menu; empty space still pans. */
        NoteArray *na = app_active_notes(app);
        bool edge = false;
        int hit = na ? find_note_at(app, mx, my, &edge) : -1;
        if (hit >= 0 && !na->notes[hit].selected) {
            for (int i = 0; i < na->count; i++) na->notes[i].selected = (i == hit);
        }
        if (selected_note_count(na) > 0) {
            ui_menu_open(NOTE_MENU, (int)(sizeof(NOTE_MENU) / sizeof(NOTE_MENU[0])),
                         mx, my, note_menu_cb, app);
        } else {
    ...
    if (app->tool == TOOL_DRAW && idx < 0) {
        ...
        if (ms < 0) ms = 0;
        double start_beat=muse_project_ms_to_beat(&app->project,ms);
        int draw_div = app->grid_snap == GRID_FREE ? 4 : grid_div(app);
        double dur=muse_project_beat_to_ms(&app->project,start_beat+1.0/draw_div)-ms;
        undo_push(&app->project);
        deselect_layer(app);
        ...
        /* Auto-expand measures to fit the note */
        {
            ...
            int needed = (int)(muse_project_ms_to_beat(&app->project,note_end_ms) / ts) + 2;
...
static void handle_mouse_motion(MuseApp *app, float mx, float my) {
    app->mouse_x = mx;
    app->mouse_y = my;
    if(app->tempo_curve_dragging&&app->tempo_curve_drag_idx>=0&&app->tempo_curve_drag_idx+1<app->project.num_tempo_points){int i=app->tempo_curve_drag_idx;MuseTempoPoint*a=&app->project.tempo_points[i],*b=&app->project.tempo_points[i+1];float linear=(a->bpm+b->bpm)*.5f,ly=app_tempo_editor_bpm_to_y(app,linear),dir=b->bpm>=a->bpm?1:-1,c=(my-ly)/42.0f*dir;if(c<-1)c=-1;if(c>1)c=1;if(SDL_GetModState()&SDL_KMOD_SHIFT)c=roundf(c*20)/20;if(fabsf(a->curve_to_next-c)>.0005f){a->curve_to_next=c;app->project.dirty=true;app->tempo_curve_drag_changed=true;set_status_msg(app,"Tempo curve: %+.2f",c);}return;}
    if(app->tempo_node_dragging&&app->tempo_node_drag_idx>=0){SDL_Keymod mods=SDL_GetModState();double beat=muse_project_ms_to_beat(&app->project,app_x_to_ms(app,mx));if(!(mods&SDL_KMOD_ALT))beat=round(beat*4.0)/4.0;if(beat<0)beat=0;float bpm=app_tempo_editor_y_to_bpm(app,my);bpm=(mods&SDL_KMOD_SHIFT)?roundf(bpm*10)/10:roundf(bpm);MuseTempoPoint*cur=&app->project.tempo_points[app->tempo_node_drag_idx];if(fabs(cur->beat-beat)>.000001||fabsf(cur->bpm-bpm)>.0005f){int ni=muse_project_move_tempo_point_raw(&app->project,app->tempo_node_drag_idx,beat,bpm);if(ni>=0){app->tempo_node_drag_idx=app->tempo_selected_idx=ni;app->tempo_node_drag_changed=true;set_status_msg(app,"Tempo: beat %.2f  %.1f BPM",beat,bpm);}}return;}
    ...
    if (app->sb_dragging > 0) {
        if (app->sb_dragging == 1) {
            ...
            double total_ms = muse_project_beat_to_ms(&app->project,(double)app->measures*beats_per_measure);
    ...
    if (app->resizing_note) {
        ...
        if (na && app->resize_note_idx >= 0 && app->resize_note_idx < na->count) {
            ...
            double nb=muse_project_ms_to_beat(&app->project,n->start);
            double min_dur=app->grid_snap==GRID_FREE?10.0:muse_project_beat_to_ms(&app->project,nb+1.0/16.0)-n->start;
            ...
            /* Auto-expand measures */
            {
                ...
                int needed = (int)(muse_project_ms_to_beat(&app->project,end_ms)/ts) + 2;
                if (needed > app->measures) app->measures = needed;
            }
        }
        return;
...
static void handle_mouse_up(MuseApp *app, int button) {
    if(button==SDL_BUTTON_LEFT&&app->tempo_curve_dragging){if(app->tempo_curve_drag_changed){double ph=muse_project_ms_to_beat(&app->tempo_curve_drag_before,app->playhead_ms),sc=muse_project_ms_to_beat(&app->tempo_curve_drag_before,app->scroll_x),la=muse_project_ms_to_beat(&app->tempo_curve_drag_before,app->loop_start_ms),lb=muse_project_ms_to_beat(&app->tempo_curve_drag_before,app->loop_end_ms);muse_project_retime_from(&app->project,&app->tempo_curve_drag_before);app->playhead_ms=muse_project_beat_to_ms(&app->project,ph);app->scroll_x=muse_project_beat_to_ms(&app->project,sc);app->loop_start_ms=muse_project_beat_to_ms(&app->project,la);app->loop_end_ms=muse_project_beat_to_ms(&app->project,lb);restart_playback_if_playing(app);}app->tempo_curve_dragging=false;app->tempo_curve_drag_changed=false;return;}
    if(button==SDL_BUTTON_LEFT&&app->tempo_node_dragging){if(app->tempo_node_drag_changed){double ph=muse_project_ms_to_beat(&app->tempo_node_drag_before,app->playhead_ms),sc=muse_project_ms_to_beat(&app->tempo_node_drag_before,app->scroll_x),la=muse_project_ms_to_beat(&app->tempo_node_drag_before,app->loop_start_ms),lb=muse_project_ms_to_beat(&app->tempo_node_drag_before,app->loop_end_ms);muse_project_retime_from(&app->project,&app->tempo_node_drag_before);app->playhead_ms=muse_project_beat_to_ms(&app->project,ph);app->scroll_x=muse_project_beat_to_ms(&app->project,sc);app->loop_start_ms=muse_project_beat_to_ms(&app->project,la);app->loop_end_ms=muse_project_beat_to_ms(&app->project,lb);restart_playback_if_playing(app);}app->tempo_node_dragging=false;app->tempo_node_drag_changed=false;return;}
...
static void humanize_commit_field(MuseApp *app) {
    if (app->humanize_edit_field < 1 || app->humanize_edit_field > 3) return;
    int value = atoi(app->humanize_edit_buf);
    int maxv[] = {50, 32, 40};
    if (value < 0) value = 0;
    if (value > maxv[app->humanize_edit_field - 1]) value = maxv[app->humanize_edit_field - 1];
    int *vals[] = {&app->humanize_timing, &app->humanize_velocity, &app->humanize_length};
    *vals[app->humanize_edit_field - 1] = value;
    app->humanize_edit_field = 0;
    SDL_StopTextInput(app->window);
}

static bool handle_humanize_event(MuseApp *app, const SDL_Event *ev) {
    if (!app->humanize_open) return false;
    float dw = 390, dh = 250;
    float dx = ((float)app->win_w - dw) * .5f;
    float dy = ((float)app->win_h - dh) * .5f;

    if (ev->type == SDL_EVENT_KEY_DOWN) {
        if (ev->key.key == SDLK_ESCAPE) {
            if (app->humanize_edit_field) humanize_commit_field(app);
            else app->humanize_open = false;
            return true;
        }
        if (app->humanize_edit_field) {
            size_t len = strlen(app->humanize_edit_buf);
            if (ev->key.key == SDLK_RETURN || ev->key.key == SDLK_KP_ENTER || ev->key.key == SDLK_TAB)
                humanize_commit_field(app);
            else if (ev->key.key == SDLK_BACKSPACE && len)
                app->humanize_edit_buf[len - 1] = '\0';
            return true;
        }
    }
    if (ev->type == SDL_EVENT_TEXT_INPUT && app->humanize_edit_field) {
        size_t len = strlen(app->humanize_edit_buf);
        for (const char *s = ev->text.text; *s && len + 1 < sizeof(app->humanize_edit_buf); s++)
            if (*s >= '0' && *s <= '9') app->humanize_edit_buf[len++] = *s;
        app->humanize_edit_buf[len] = '\0';
        return true;
    }
    if (ev->type == SDL_EVENT_MOUSE_MOTION) {
        app->mouse_x = ev->motion.x; app->mouse_y = ev->motion.y;
        if (app->humanize_drag_slider) {
            int i = app->humanize_drag_slider - 1;
            int maxv[] = {50, 32, 40};
            float frac = (ev->motion.x - app->_humanize_sliders[i].x - 10) /
                         (app->_humanize_sliders[i].w - 20);
            if (frac < 0) frac = 0; if (frac > 1) frac = 1;
            int *vals[] = {&app->humanize_timing, &app->humanize_velocity, &app->humanize_length};
            *vals[i] = (int)lroundf(frac * maxv[i]);
        }
        return true;
    }
    if (ev->type == SDL_EVENT_MOUSE_BUTTON_UP) {
        app->humanize_drag_slider = 0;
        return true;
    }
    if (ev->type == SDL_EVENT_MOUSE_BUTTON_DOWN && ev->button.button == SDL_BUTTON_LEFT) {
        float mx = ev->button.x, my = ev->button.y;
        if (app->humanize_edit_field) humanize_commit_field(app);
        if (mx >= dx + dw - 32 && mx <= dx + dw - 10 && my >= dy + 7 && my <= dy + 27) {
            app->humanize_open = false; return true;
        }
        for (int i = 0; i < 3; i++) {
            if (ui_rect_contains(app->_humanize_fields[i], mx, my)) {
                int vals[] = {app->humanize_timing, app->humanize_velocity, app->humanize_length};
                app->humanize_edit_field = i + 1;
                snprintf(app->humanize_edit_buf, sizeof(app->humanize_edit_buf), "%d", vals[i]);
                SDL_StartTextInput(app->window); return true;
            }
            if (ui_rect_contains(app->_humanize_sliders[i], mx, my)) {
                app->humanize_drag_slider = i + 1;
                int maxv[] = {50, 32, 40};
                float frac = (mx - app->_humanize_sliders[i].x - 10) /
                             (app->_humanize_sliders[i].w - 20);
                if (frac < 0) frac = 0; if (frac > 1) frac = 1;
                int *vals[] = {&app->humanize_timing, &app->humanize_velocity, &app->humanize_length};
                *vals[i] = (int)lroundf(frac * maxv[i]); return true;
            }
        }
        UiRect apply = {dx + dw - 116, dy + dh - 42, 96, 28};
        UiRect cancel = {dx + dw - 214, dy + dh - 42, 88, 28};
        if (ui_rect_contains(apply, mx, my)) {
            NoteArray *na = app_active_notes(app);
            undo_push(&app->project);
            int n = notes_humanize(na, app->humanize_timing, app->humanize_velocity,
                                   app->humanize_length, (uint32_t)SDL_GetTicks());
            if (n) { app->project.dirty = true; auto_expand_measures(app); restart_playback_if_playing(app); }
            set_status_msg(app, "Humanized %d notes (timing %d ms, velocity %d, length %d%%)",
                           n, app->humanize_timing, app->humanize_velocity, app->humanize_length);
            app->humanize_open = false; return true;
        }
        if (ui_rect_contains(cancel, mx, my) || mx < dx || mx > dx + dw || my < dy || my > dy + dh)
            app->humanize_open = false;
        return true;
    }
    return true;
}

static void render_humanize_dialog(MuseApp *app) {
    if (!app->humanize_open) return;
    SDL_Renderer *r = app->renderer;
    float dw = 390, dh = 250;
    float dx = ((float)app->win_w - dw) * .5f;
    float dy = ((float)app->win_h - dh) * .5f;
    draw_filled_rect(r, 0, 0, (float)app->win_w, (float)app->win_h, 0, 0, 0, 0x80);
    DialogFrame df = draw_dialog_frame(r, dx, dy, dw, dh, "Humanize Selection", app->mouse_x, app->mouse_y);
    const char *labels[] = {"Timing", "Velocity", "Note length"};
    const char *units[] = {"ms", "", "%"};
    int vals[] = {app->humanize_timing, app->humanize_velocity, app->humanize_length};
    int maxv[] = {50, 32, 40};
    for (int i = 0; i < 3; i++) {
        float y = df.content_y + 12 + i * 48;
        draw_text(r, labels[i], dx + 18, y + 5, 11, COL_TEXT);
        UiRect slider = {dx + 112, y - 3, 180, 26};
        app->_humanize_sliders[i] = slider;
        draw_ctk_slider(r, slider.x, y, slider.w, (float)vals[i], 0, (float)maxv[i],
                        app->humanize_drag_slider == i + 1);
        UiRect field = {dx + 306, y - 3, 58, 26};
        app->_humanize_fields[i] = field;
        bool active = app->humanize_edit_field == i + 1;
        draw_rounded_rect(r, field.x, field.y, field.w, field.h, 4,
                          active ? COL_BG_LIGHT : COL_BG, 0xFF);
        draw_rounded_rect_outline(r, field.x, field.y, field.w, field.h, 4,
                                  active ? COL_GOLD : COL_BORDER);
        char value[24];
        if (active) snprintf(value, sizeof(value), "%s", app->humanize_edit_buf);
        else snprintf(value, sizeof(value), "%d %s", vals[i], units[i]);
        draw_text_centered(r, value, field.x + field.w * .5f, field.y + field.h * .5f, 10,
                           active ? COL_GOLD_LIGHT : COL_TEXT);
    }
    draw_text(r, "Each amount is a random +/- range. Zero disables that variation.",
              dx + 18, dy + dh - 70, 9, COL_TEXT_DIM);
    UiRect cancel = {dx + dw - 214, dy + dh - 42, 88, 28};
    UiRect apply = {dx + dw - 116, dy + dh - 42, 96, 28};
    draw_ctk_button(r, cancel, "Cancel", 11, ui_rect_contains(cancel, app->mouse_x, app->mouse_y), false);
    draw_ctk_button(r, apply, "Humanize", 11, ui_rect_contains(apply, app->mouse_x, app->mouse_y), false);
}

static void velocity_set_commit_text(MuseApp *app){
    if(!app->velocity_set_editing)return;int v=atoi(app->velocity_set_buf);if(v<1)v=1;if(v>127)v=127;
    app->velocity_set_value=v;app->velocity_set_editing=false;SDL_StopTextInput(app->window);
}
static bool handle_velocity_set_event(MuseApp*app,const SDL_Event*ev){
    if(!app->velocity_set_open)return false;
    float dw=350,dh=175,dx=((float)app->win_w-dw)*.5f,dy=((float)app->win_h-dh)*.5f;
    if(ev->type==SDL_EVENT_KEY_DOWN){
        if(ev->key.key==SDLK_ESCAPE){if(app->velocity_set_editing)velocity_set_commit_text(app);else app->velocity_set_open=false;return true;}
        if(app->velocity_set_editing){size_t n=strlen(app->velocity_set_buf);if(ev->key.key==SDLK_RETURN||ev->key.key==SDLK_KP_ENTER||ev->key.key==SDLK_TAB)velocity_set_commit_text(app);else if(ev->key.key==SDLK_BACKSPACE&&n)app->velocity_set_buf[n-1]=0;return true;}
    }
    if(ev->type==SDL_EVENT_TEXT_INPUT&&app->velocity_set_editing){size_t n=strlen(app->velocity_set_buf);for(const char*s=ev->text.text;*s&&n+1<sizeof(app->velocity_set_buf);s++)if(*s>='0'&&*s<='9')app->velocity_set_buf[n++]=*s;app->velocity_set_buf[n]=0;return true;}
    if(ev->type==SDL_EVENT_MOUSE_MOTION){app->mouse_x=ev->motion.x;app->mouse_y=ev->motion.y;if(app->velocity_set_dragging){float f=(ev->motion.x-app->_velocity_set_slider.x-10)/(app->_velocity_set_slider.w-20);if(f<0)f=0;if(f>1)f=1;app->velocity_set_value=1+(int)lroundf(f*126);}return true;}
    if(ev->type==SDL_EVENT_MOUSE_BUTTON_UP){app->velocity_set_dragging=false;return true;}
    if(ev->type==SDL_EVENT_MOUSE_BUTTON_DOWN&&ev->button.button==SDL_BUTTON_LEFT){float mx=ev->button.x,my=ev->button.y;if(app->velocity_set_editing)velocity_set_commit_text(app);
        if(mx>=dx+dw-32&&mx<=dx+dw-10&&my>=dy+7&&my<=dy+27){app->velocity_set_open=false;return true;}
        if(ui_rect_contains(app->_velocity_set_field,mx,my)){app->velocity_set_editing=true;snprintf(app->velocity_set_buf,sizeof(app->velocity_set_buf),"%d",app->velocity_set_value);SDL_StartTextInput(app->window);return true;}
        if(ui_rect_contains(app->_velocity_set_slider,mx,my)){app->velocity_set_dragging=true;float f=(mx-app->_velocity_set_slider.x-10)/(app->_velocity_set_slider.w-20);if(f<0)f=0;if(f>1)f=1;app->velocity_set_value=1+(int)lroundf(f*126);return true;}
        UiRect apply={dx+dw-116,dy+dh-42,96,28},cancel={dx+dw-214,dy+dh-42,88,28};
        if(ui_rect_contains(apply,mx,my)){NoteArray*na=app_active_notes(app);int count=selected_note_count(na);if(count){undo_push(&app->project);for(int i=0;i<na->count;i++)if(na->notes[i].selected)na->notes[i].vel=(uint8_t)app->velocity_set_value;app->project.dirty=true;restart_playback_if_playing(app);set_status_msg(app,"Set velocity to %d on %d notes",app->velocity_set_value,count);}app->velocity_set_open=false;return true;}
        if(ui_rect_contains(cancel,mx,my)||mx<dx||mx>dx+dw||my<dy||my>dy+dh)app->velocity_set_open=false;return true;
    }
    return true;
}
static void render_velocity_set_dialog(MuseApp*app){
    if(!app->velocity_set_open)return;SDL_Renderer*r=app->renderer;float dw=350,dh=175,dx=((float)app->win_w-dw)*.5f,dy=((float)app->win_h-dh)*.5f;
    draw_filled_rect(r,0,0,(float)app->win_w,(float)app->win_h,0,0,0,0x80);DialogFrame df=draw_dialog_frame(r,dx,dy,dw,dh,"Set Selection Velocity",app->mouse_x,app->mouse_y);
    draw_text(r,"Velocity",dx+18,df.content_y+25,11,COL_TEXT);app->_velocity_set_slider=(UiRect){dx+85,df.content_y+16,175,26};draw_ctk_slider(r,app->_velocity_set_slider.x,df.content_y+19,app->_velocity_set_slider.w,(float)app->velocity_set_value,1,127,app->velocity_set_dragging);
    app->_velocity_set_field=(UiRect){dx+274,df.content_y+16,54,26};bool active=app->velocity_set_editing;draw_rounded_rect(r,app->_velocity_set_field.x,app->_velocity_set_field.y,app->_velocity_set_field.w,app->_velocity_set_field.h,4,active?COL_BG_LIGHT:COL_BG,255);draw_rounded_rect_outline(r,app->_velocity_set_field.x,app->_velocity_set_field.y,app->_velocity_set_field.w,app->_velocity_set_field.h,4,active?COL_GOLD:COL_BORDER);char value[8];snprintf(value,sizeof(value),"%s",active?app->velocity_set_buf:"");if(!active)snprintf(value,sizeof(value),"%d",app->velocity_set_value);draw_text_centered(r,value,app->_velocity_set_field.x+27,app->_velocity_set_field.y+13,10,active?COL_GOLD_LIGHT:COL_TEXT);
    UiRect cancel={dx+dw-214,dy+dh-42,88,28},apply={dx+dw-116,dy+dh-42,96,28};draw_ctk_button(r,cancel,"Cancel",11,ui_rect_contains(cancel,app->mouse_x,app->mouse_y),false);draw_ctk_button(r,apply,"Apply",11,ui_rect_contains(apply,app->mouse_x,app->mouse_y),false);
}
...
static bool handle_midi_dlg_event(MuseApp *app, const SDL_Event *ev) {
    ...
    if (ev->type == SDL_EVENT_MOUSE_BUTTON_DOWN &&
        ...
        if (ui_rect_contains(import_btn, mx, my)) {
            ...
            if(app->project.num_tempo_points>1)app->tempo_lane_open=true;
            app->playhead_ms = 0;
            app->scroll_x = 0;
            /* Calculate measures from note data */
            double max_end = 0;
            ...
            int beats=(int)muse_project_ms_to_beat(&app->project,max_end)+4;
            int ts = app->project.time_sig ? app->project.time_sig : 4;
            app->measures = (beats + ts - 1) / ts;
            if (app->measures < 4) app->measures = 4;
            {
                ...
                if(mid->tempo_changes>1)set_status_msg(app,"Imported %d notes and %d tempo graph points",total_notes,app->project.num_tempo_points);
                else set_status_msg(app, "Imported: %s (%d notes)  |  Ctrl+M to re-import", app->filename, total_notes);
            }
...
void muse_app_handle_event(MuseApp *app, const SDL_Event *ev) {
    ...
    /* Context menu is topmost and consumes its own keyboard/pointer input. */
    if (ui_menu_handle_event(ev)) return;
    ...
    if (handle_velocity_set_event(app, ev)) return;
    if (handle_humanize_event(app, ev)) return;
    ...
    case SDL_EVENT_MOUSE_BUTTON_DOWN: {
        ...
        if (app->edit_field != 0 && ev->button.button == SDL_BUTTON_LEFT) {
            ...
            if (!ui_rect_contains(*er, mx, my)) {
                ...
                if (app->edit_field == 1) {
                    ...
                    app_change_base_bpm(app,(uint16_t)val);
        ...
        if(mx>=rx-KEYS_WIDTH&&mx<rx&&my>=hdr_y&&my<hdr_y+HEADER_HEIGHT&&ev->button.button==SDL_BUTTON_LEFT){
            app->tempo_lane_open=!app->tempo_lane_open;
            set_status_msg(app,app->tempo_lane_open?"Tempo graph expanded":"Tempo graph collapsed");
            break;
        }
        if(app->tempo_lane_open&&mx>=rx&&mx<rx+rw&&my>=hdr_y+HEADER_HEIGHT&&my<hdr_y+app_timeline_header_h(app)){
            double beat=muse_project_ms_to_beat(&app->project,app_x_to_ms(app,mx));
            beat=round(beat*4.0)/4.0;if(beat<0)beat=0;
            double playbeat=muse_project_ms_to_beat(&app->project,app->playhead_ms);
            double scrollbeat=muse_project_ms_to_beat(&app->project,app->scroll_x);
            double loopa=muse_project_ms_to_beat(&app->project,app->loop_start_ms),loopb=muse_project_ms_to_beat(&app->project,app->loop_end_ms);
            bool changed=false;
            if(ev->button.button==SDL_BUTTON_LEFT){for(int i=0;i+1<app->project.num_tempo_points;i++){MuseTempoPoint*a=&app->project.tempo_points[i],*b=&app->project.tempo_points[i+1];if(!a->ramp_to_next)continue;double u=.5,sh=u;if(a->curve_to_next>0)sh=1-pow(1-u,1+4*a->curve_to_next);else if(a->curve_to_next<0)sh=pow(u,1-4*a->curve_to_next);float hx=app_ms_to_x(app,muse_project_beat_to_ms(&app->project,(a->beat+b->beat)*.5)),hy=app_tempo_editor_bpm_to_y(app,a->bpm+(b->bpm-a->bpm)*(float)sh),dx=mx-hx,dy=my-hy;if(dx*dx+dy*dy<=81){undo_push(&app->project);app->tempo_curve_drag_before=app->project;app->tempo_curve_drag_changed=false;app->tempo_curve_dragging=true;app->tempo_curve_drag_idx=i;set_status_msg(app,"Drag curve handle (Shift: fine)");break;}}if(app->tempo_curve_dragging)break;}
            int hit=-1;float best=10;for(int i=0;i<app->project.num_tempo_points;i++){float tx=app_ms_to_x(app,muse_project_beat_to_ms(&app->project,app->project.tempo_points[i].beat)),ty=app_tempo_editor_bpm_to_y(app,app->project.tempo_points[i].bpm),dx=mx-tx,dy=my-ty,d=sqrtf(dx*dx+dy*dy);if(d<best){best=d;hit=i;}}
            if(ev->button.button==SDL_BUTTON_RIGHT){undo_push(&app->project);changed=muse_project_remove_tempo_at_beat(&app->project,hit>=0?app->project.tempo_points[hit].beat:beat,hit>=0?0.001:0.30);app->tempo_selected_idx=-1;}
            else if(ev->button.button==SDL_BUTTON_LEFT&&hit>=0){app->tempo_selected_idx=hit;if(ev->button.clicks>=2){app->edit_field=10;snprintf(app->edit_buf,sizeof(app->edit_buf),"%.2f",app->project.tempo_points[hit].bpm);SDL_StartTextInput(app->window);set_status_msg(app,"Type exact BPM and press Enter");break;}if(SDL_GetModState()&SDL_KMOD_CTRL){undo_push(&app->project);changed=muse_project_set_tempo_ramp(&app->project,hit,!app->project.tempo_points[hit].ramp_to_next);set_status_msg(app,"Tempo segment: %s",app->project.tempo_points[hit].ramp_to_next?"ramp":"step");}else{undo_push(&app->project);app->tempo_node_drag_before=app->project;app->tempo_node_drag_changed=false;app->tempo_node_dragging=true;app->tempo_node_drag_idx=hit;set_status_msg(app,"Drag tempo node (Alt: free time, Shift: 0.1 BPM)");break;}}
            else if(ev->button.button==SDL_BUTTON_LEFT){SDL_Keymod mods=SDL_GetModState();float bpm=app_tempo_editor_y_to_bpm(app,my);bpm=(mods&SDL_KMOD_SHIFT)?roundf(bpm*10)/10:roundf(bpm);undo_push(&app->project);changed=muse_project_set_tempo_at_beat(&app->project,beat,bpm);int prev=-1;for(int i=0;i<app->project.num_tempo_points;i++){if(fabs(app->project.tempo_points[i].beat-beat)<1e-6)app->tempo_selected_idx=i;if(app->project.tempo_points[i].beat<beat-1e-9)prev=i;}if(changed&&!(mods&SDL_KMOD_CTRL)){if(prev<0&&beat>1e-9){muse_project_set_tempo_at_beat(&app->project,0,(float)app->project.bpm);prev=0;app->tempo_selected_idx++;}if(prev>=0)muse_project_set_tempo_ramp(&app->project,prev,true);}}
            if(changed){app->playhead_ms=muse_project_beat_to_ms(&app->project,playbeat);app->scroll_x=muse_project_beat_to_ms(&app->project,scrollbeat);app->loop_start_ms=muse_project_beat_to_ms(&app->project,loopa);app->loop_end_ms=muse_project_beat_to_ms(&app->project,loopb);restart_playback_if_playing(app);set_status_msg(app,ev->button.button==SDL_BUTTON_RIGHT?"Tempo point removed":"Tempo point at beat %.2f",beat);}
            break;
        }
    ...
    case SDL_EVENT_KEY_DOWN: {
        ...
        if (app->edit_field != 0) {
            ...
            if (k == SDLK_RETURN || k == SDLK_KP_ENTER || k == SDLK_ESCAPE || k == SDLK_TAB) {
                ...
                if (k != SDLK_ESCAPE) {
                    if (app->edit_field == 1) {
                        ...
                        if (app->edit_buf[0])
                            snprintf(app->filename, sizeof(app->filename), "%s", app->edit_buf);
                    } else if(app->edit_field==10&&app->tempo_selected_idx>=0&&app->tempo_selected_idx<app->project.num_tempo_points){float bpm=(float)atof(app->edit_buf);if(bpm<20)bpm=20;if(bpm>400)bpm=400;undo_push(&app->project);muse_project_move_tempo_point(&app->project,app->tempo_selected_idx,app->project.tempo_points[app->tempo_selected_idx].beat,bpm);
                    } else if (app->edit_field >= 4 && app->edit_field <= 5 && app->midi_dlg_data) {
                        MidiImportData *vmid = (MidiImportData *)app->midi_dlg_data;
                        int val = atoi(app->edit_buf);
                        if (val < 1) val = 1; if (val > 127) val = 127;
            ...
            } else if((k==SDLK_PERIOD||k==SDLK_KP_DECIMAL)&&app->edit_field==10&&!strchr(app->edit_buf,'.')){int len=(int)strlen(app->edit_buf);if(len<6){app->edit_buf[len]='.';app->edit_buf[len+1]=0;}}
        ...
        case SDLK_DELETE:
        case SDLK_BACKSPACE:
            if(app->tempo_lane_open&&app->tempo_selected_idx>=0&&app->tempo_selected_idx<app->project.num_tempo_points){double beat=app->project.tempo_points[app->tempo_selected_idx].beat;undo_push(&app->project);muse_project_remove_tempo_at_beat(&app->project,beat,1e-6);app->tempo_selected_idx=-1;set_status_msg(app,"Tempo point removed");}else delete_selected(app);
            break;
        case SDLK_A:
            if (ctrl && alt) {
                NoteArray *na=app_active_notes(app);if(na){undo_push(&app->project);int n=notes_arpeggiate(na,ms_per_beat(app),4,1,80);if(n){app->project.dirty=true;auto_expand_measures(app);restart_playback_if_playing(app);}set_status_msg(app,"Arpeggiated: %d notes",n);}
            } else if (ctrl) {
                {
                    NoteArray *na = app_active_notes(app);
        ...
        case SDLK_S:
            if (ctrl && alt) {
                NoteArray *na=app_active_notes(app);if(na){undo_push(&app->project);int n=notes_strum(na,25,4,2,(uint32_t)SDL_GetTicks(),true);if(n){app->project.dirty=true;restart_playback_if_playing(app);}set_status_msg(app,"Strummed up: %d notes",n);}
            } else if (ctrl) {
                ui_toolbar_save_project(app);
            } else {
                /* S = select tool */
        ...
        case SDLK_L:
            ...
            break;
        case SDLK_Q:
            if(ctrl&&alt){NoteArray*na=app_active_notes(app);if(na){undo_push(&app->project);int n=notes_quantize(na,ms_per_beat(app),grid_div(app),100,50,false);if(n){app->project.dirty=true;restart_playback_if_playing(app);}set_status_msg(app,"Quantized: %d notes",n);}}
            break;
        case SDLK_H:
            if(ctrl&&alt){NoteArray*na=app_active_notes(app);if(na){undo_push(&app->project);int n=notes_humanize(na,12,8,10,(uint32_t)SDL_GetTicks());if(n){app->project.dirty=true;restart_playback_if_playing(app);}set_status_msg(app,"Humanized: %d notes",n);}}
            break;
        case SDLK_HOME:
            app->playhead_ms = 0;
            app->scroll_x = 0;
            break;
        case SDLK_K:
            if(ctrl&&app->project.num_markers<MAX_ARRANGEMENT_MARKERS){undo_push(&app->project);int i=app->project.num_markers++;app->project.markers[i].time_ms=app->playhead_ms;snprintf(app->project.markers[i].label,sizeof(app->project.markers[i].label),"Marker %d",i+1);app->project.dirty=true;set_status_msg(app,"Added %s",app->project.markers[i].label);}
            break;
        case SDLK_R:
            if(ctrl){app->midi_recording=!app->midi_recording;set_status_msg(app,"MIDI recording: %s",app->midi_recording?"ON":"OFF");}
            break;
        case SDLK_I:
            if (ctrl && shift) {
        ...
        case SDLK_LEFT: {
            NoteArray *na = app_active_notes(app);
            if (na) {
                double nudge = app->grid_snap == GRID_FREE ? 1.0 : ms_per_beat(app) / grid_div(app);
                undo_push(&app->project);
                for (int i = 0; i < na->count; i++) {
                    if (na->notes[i].selected) {
        ...
        case SDLK_M:
            if (ctrl && shift) {
                SDL_CreateDirectory("converted");
                char path[512];
                snprintf(path,sizeof(path),"converted/%s.mid",app->filename);
                int rc=midi_export(path,&app->project);
                if(rc==0)set_status_msg(app,"Exported MIDI: %s",path);
                else set_status_msg(app,"MIDI export failed (%d)",rc);
            } else if (ctrl && app->midi_dlg_data && !app->midi_dlg_open) {
...
static void fx_tick(MuseApp *app) {
    ...
    app->anim_time = now;
    {float target=app->tempo_lane_open?1.0f:0.0f,step=app->dt*7.5f;if(app->tempo_lane_anim<target)app->tempo_lane_anim=fminf(target,app->tempo_lane_anim+step);else if(app->tempo_lane_anim>target)app->tempo_lane_anim=fmaxf(target,app->tempo_lane_anim-step);}
...
void muse_app_render(MuseApp *app) {
    ...
    MidiInputEvent mev[64];int mcount=midi_input_poll(mev,64);
    for(int i=0;i<mcount;i++){int pitch=mev[i].note;if(pitch<0||pitch>127)continue;MuseLayer*ly=app->project.num_layers?&app->project.layers[app->project.active_layer]:NULL;if(!ly)continue;
        if(mev[i].status==0x90&&mev[i].velocity){muse_audio_preview(pitch,mev[i].velocity,1000,ly->inst_id,app->cur_ntype,ly->synth_profile);if(app->midi_recording){app->midi_note_start[pitch]=app->playhead_ms+1;app->midi_note_velocity[pitch]=mev[i].velocity;}}
        else if(app->midi_recording&&app->midi_note_start[pitch]>0){double st=app->midi_note_start[pitch]-1,dur=app->playhead_ms-st;if(dur<10)dur=10;NoteArray*na=app_active_notes(app);if(na){undo_push(&app->project);MuseNote n={(uint8_t)pitch,app->midi_note_velocity[pitch],app->cur_ntype,0,st,dur};note_array_push(na,n);app->project.dirty=true;}app->midi_note_start[pitch]=0;}}
    ...
    if (app->playing) {
        ...
        if (ph_x > roll_rx + roll_rw - 10) {
            double phb=muse_project_ms_to_beat(&app->project,app->playhead_ms);
            app->scroll_x=muse_project_beat_to_ms(&app->project,phb-(double)roll_rw/beat_px(app)*0.15);
        }

        /* Check if playback ended naturally */
        double total_dur_ms = muse_project_beat_to_ms(&app->project,
                              (double)app->measures*app->project.time_sig);
    ...
    render_humanize_dialog(app);
    render_velocity_set_dialog(app);
    ...
    if (app->shortcuts_open) {
        ...
        SC_ROW("Ctrl+Alt+Q",     "Quantize selection");
        SC_ROW("Ctrl+Alt+H",     "Humanize selection");
        SC_ROW("Ctrl+Alt+A",     "Arpeggiate selection");
        SC_ROW("Ctrl+Alt+S",     "Strum selection");
        ly += 6;
        ...
        SC_ROW("Middle-Click",    "Note menu / pan");
        ly += 6;
        ...
        SC_ROW("Ctrl+Shift+M",   "Export MIDI");
        SC_ROW("Ctrl+K",         "Add marker");
        SC_ROW("Ctrl+R",         "Record MIDI input");
    ...
    /* Context menu is the topmost ordinary editor surface. */
    ui_menu_render(r, app->mouse_x, app->mouse_y);
    ...
    if (app->overlay_state > 0) {
        ...
        if (oa > 0) {
            ...
            bool direct_gpu = gpu_active();
            SDL_Texture *ov_tex = direct_gpu ? NULL :
                SDL_CreateTexture(r, SDL_PIXELFORMAT_RGBA32,
                    SDL_TEXTUREACCESS_TARGET, (int)ww, (int)wh);
            if (direct_gpu || ov_tex) {
                if (ov_tex) {
