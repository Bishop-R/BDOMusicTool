...
/* Editable projects always live in Save/.  Dialog paths are used only to
   obtain the requested filename, keeping Linux and Windows behavior equal. */
static void composer_save_path(char *out, size_t cap, const char *chosen) {
    const char *name = chosen;
    for (const char *p = chosen; *p; p++)
        if (*p == '/' || *p == '\\') name = p + 1;
    size_t len = strlen(name);
    SDL_CreateDirectory("Save");
    if (len > 9 && strcmp(name + len - 9, ".composer") == 0)
        snprintf(out, cap, "Save/%s", name);
    else if (len > 4 && strcmp(name + len - 4, ".bdo") == 0)
        snprintf(out, cap, "Save/%.*s.composer", (int)(len - 4), name);
    else
        snprintf(out, cap, "Save/%s.composer", name);
}
...
static void open_callback(void *userdata, const char *const *filelist, int filter) {
    ...
    if (is_midi) {
        ...
        if (rc == 0) {
            composer_save_path(app->project_path, sizeof(app->project_path), path);
            const char *opened_name = path;
            for (const char *p = path; *p; p++)
                if (*p == '/' || *p == '\\') opened_name = p + 1;
            snprintf(app->filename, sizeof(app->filename), "%s", opened_name);
            int total_notes = 0;
...
static void save_callback(void *userdata, const char *const *filelist, int filter) {
    ...
    if (!filelist || !filelist[0]) return;
    char spath[1024];
    /* Save is exclusively the editable project format.  BDO game output is
       produced by Export and is extensionless. */
    composer_save_path(spath, sizeof(spath), filelist[0]);
    ...
    for (const char *p = spath; *p; p++)
        if (*p == '/' || *p == '\\') f = p + 1;
    char previous_filename[sizeof(app->filename)];
    snprintf(previous_filename, sizeof(previous_filename), "%s", app->filename);
    snprintf(app->filename, sizeof(app->filename), "%s", f);
    int rc = muse_save(spath, app);

    if (rc == 0) {
        snprintf(app->project_path, sizeof(app->project_path), "%s", spath);
        app->project.dirty = false;
        set_status_msg(app, "Saved: %s", app->filename);
    } else snprintf(app->filename, sizeof(app->filename), "%s", previous_filename);
}
...
void ui_toolbar_save_as(MuseApp *app) {
    SDL_ShowSaveFileDialog(save_callback, app, app->window, save_filters, 1, "Save");
}
void ui_toolbar_save_project(MuseApp *app) {
    /* Normal Save is intentionally dialog-free.  The editable title/name
       field supplies the basename and composer_save_path anchors it in Save/. */
    const char *name = app->filename[0] ? app->filename : "untitled";
    const char *filelist[] = {name, NULL};
    save_callback(app, filelist, 0);
}
