...
#define MAX_ARRANGEMENT_MARKERS 64
typedef struct { double time_ms; char label[32]; } MuseMarker;
#define MAX_TEMPO_POINTS 64
typedef struct { double beat; float bpm; uint8_t ramp_to_next; float curve_to_next; } MuseTempoPoint;
...
typedef struct {
    ...
    char       char_name[64];
    bool       dirty;
    MuseMarker markers[MAX_ARRANGEMENT_MARKERS];
    int        num_markers;
    MuseTempoPoint tempo_points[MAX_TEMPO_POINTS];
    int        num_tempo_points;
...
double muse_project_ms_to_beat(const MuseProject *p, double ms);
...
bool   muse_project_set_base_bpm(MuseProject *p, uint16_t bpm);
int    muse_project_move_tempo_point(MuseProject *p,int index,double beat,float bpm);
int    muse_project_move_tempo_point_raw(MuseProject *p,int index,double beat,float bpm);
bool   muse_project_set_tempo_ramp(MuseProject *p,int index,bool enabled);
bool   muse_project_set_tempo_curve(MuseProject *p,int index,float curve);
void   muse_project_retime_from(MuseProject *p,const MuseProject *old_tempo);
