...
int muse_audio_export_wav(const char *path, MuseProject *proj, int measures) {
    ...
    /* Export finalization: remove DC, protect file endpoints, then apply
       deterministic TPDF dither while reducing to 16-bit PCM. */
    float x1[2]={0},y1[2]={0};
    int fade_frames=SAMPLE_RATE/200; /* 5 ms */
    for(int f=0;f<total_frames;f++)for(int c=0;c<2;c++){
        int i=f*2+c;float x=buf[i];float y=x-x1[c]+0.995f*y1[c];x1[c]=x;y1[c]=y;
        float gain=1.0f;if(f<fade_frames)gain=(float)f/fade_frames;if(f>=total_frames-fade_frames){float g=(float)(total_frames-1-f)/fade_frames;if(g<gain)gain=g;}buf[i]=y*gain;
    }

    /* Convert to int16 */
    int16_t *pcm = malloc((size_t)total_frames * 2 * sizeof(int16_t));
    if (!pcm) { free(buf); return -1; }
    uint32_t dither_state=0x42444f4du;
    for (int i = 0; i < total_frames * 2; i++) {
        dither_state=dither_state*1664525u+1013904223u;float d1=(float)(dither_state&0xffff)/65535.0f;
        dither_state=dither_state*1664525u+1013904223u;float d2=(float)(dither_state&0xffff)/65535.0f;
        float s = buf[i] * 32000.0f + (d1-d2);
