...
static Snapshot snapshot_copy(const MuseProject *p) {
    ...
    s.num_markers  = p->num_markers;
    memcpy(s.markers,p->markers,sizeof(s.markers));
    s.num_tempo_points=p->num_tempo_points;
    memcpy(s.tempo_points,p->tempo_points,sizeof(s.tempo_points));
...
static void snapshot_restore(const Snapshot *s, MuseProject *p) {
    ...
    p->num_markers  = s->num_markers;
    memcpy(p->markers,s->markers,sizeof(p->markers));
    p->num_tempo_points=s->num_tempo_points;
    memcpy(p->tempo_points,s->tempo_points,sizeof(p->tempo_points));
    p->dirty        = true;
