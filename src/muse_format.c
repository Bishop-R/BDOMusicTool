...
int muse_save(const char *path, const MuseApp *app) {
    char temp_path[1100];
    if (!path || strlen(path) + 5 >= sizeof(temp_path)) return -1;
    snprintf(temp_path, sizeof(temp_path), "%s.tmp", path);
    FILE *f = fopen_utf8(temp_path, "w");
    if (!f) return -1;
    ...
    fprintf(f, "  \"markers\": [");
    for(int i=0;i<p->num_markers;i++){if(i)fprintf(f,",");fprintf(f,"[%.2f,",p->markers[i].time_ms);write_json_string(f,p->markers[i].label);fprintf(f,"]");}
    fprintf(f, "],\n");
    fprintf(f, "  \"tempo_points\": [");
    for(int i=0;i<p->num_tempo_points;i++){if(i)fprintf(f,",");fprintf(f,"[%.6f,%.2f,%d,%.3f]",p->tempo_points[i].beat,p->tempo_points[i].bpm,p->tempo_points[i].ramp_to_next,p->tempo_points[i].curve_to_next);}
    fprintf(f, "],\n");
    ...
    /* Windows rename cannot replace an existing file. Keep a recoverable
       .bak until the new, complete file is in place. */
    char backup_path[1100];
    snprintf(backup_path, sizeof(backup_path), "%s.bak", path);
    remove(backup_path);
    bool had_old = rename(path, backup_path) == 0;
    if (rename(temp_path, path) != 0) {
        if (had_old) rename(backup_path, path);
        remove(temp_path);
        return -3;
    }
    if (had_old) remove(backup_path);
    return 0;
}
...
int muse_load(const char *path, MuseApp *app) {
    ...
    while (*p && *p != '}') {
        ...
        if (strcmp(key, "bpm") == 0) {
            ...
            p=jskip(p);if(*p=='[')p=jskip(p+1);
            while(*p&&*p!=']'&&app->project.num_markers<MAX_ARRANGEMENT_MARKERS){if(*p!='['){p=jskip_value(p);break;}p=jskip(p+1);
                MuseMarker*m=&app->project.markers[app->project.num_markers];p=jnum(p,&m->time_ms);p=jskip(p);if(*p==',')p=jskip(p+1);p=jstr(p,m->label,sizeof(m->label));p=jskip(p);if(*p==']')p=jskip(p+1);app->project.num_markers++;p=jskip(p);if(*p==',')p=jskip(p+1);}
            if(*p==']')p=jskip(p+1);
        } else if (strcmp(key, "tempo_points") == 0) {
            p=jskip(p);if(*p=='[')p=jskip(p+1);
            while(*p&&*p!=']'&&app->project.num_tempo_points<MAX_TEMPO_POINTS){if(*p!='['){p=jskip_value(p);break;}p=jskip(p+1);
                MuseTempoPoint*t=&app->project.tempo_points[app->project.num_tempo_points];double bpm=120,ramp=0,curve=0;p=jnum(p,&t->beat);p=jskip(p);if(*p==',')p=jskip(p+1);p=jnum(p,&bpm);t->bpm=(float)bpm;p=jskip(p);if(*p==','){p=jskip(p+1);p=jnum(p,&ramp);t->ramp_to_next=(uint8_t)ramp;p=jskip(p);if(*p==','){p=jskip(p+1);p=jnum(p,&curve);t->curve_to_next=(float)curve;p=jskip(p);}}if(*p==']')p=jskip(p+1);app->project.num_tempo_points++;p=jskip(p);if(*p==',')p=jskip(p+1);}
            if(*p==']')p=jskip(p+1);
